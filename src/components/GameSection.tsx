    ...
    >
      ...
      <div className="relative z-20 container mx-auto flex flex-col items-center lg:items-start justify-center w-full h-full pb-40 lg:pb-0 px-4 lg:px-12 xl:px-20">
        ...
        <div className="relative z-20 w-full max-w-4xl lg:max-w-3xl xl:max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 lg:mt-0 lg:self-start">
          {/* Card 1 */}
          <div
            className={`backdrop-blur-md border-2 border-yellow-500/50 rounded-2xl p-6 xl:p-8 flex flex-col items-center justify-center text-center gap-3 transition-all duration-700 delay-300 shadow-lg transform ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
            }`}
          >
            {/* Removed group-hover effect, keeping it white to distinguish or you can make it text-yellow-500 */}
            <Gamepad2 className="w-10 h-10 xl:w-14 xl:h-14 text-white" />
            <span className="text-white font-bold text-lg xl:text-2xl">Play the Game</span>
          </div>

          {/* Card 2 */}
          <div
            className={`backdrop-blur-md border-2 border-yellow-500/50 rounded-2xl p-6 xl:p-8 flex flex-col items-center justify-center text-center gap-3 transition-all duration-700 delay-500 shadow-lg transform ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
            }`}
          >
          ...
          {/* Card 3 */}
          <div
            className={`backdrop-blur-md border-2 border-yellow-500/50 rounded-2xl p-6 xl:p-8 flex flex-col items-center justify-center text-center gap-3 transition-all duration-700 delay-700 shadow-lg transform ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
            }`}
          >
